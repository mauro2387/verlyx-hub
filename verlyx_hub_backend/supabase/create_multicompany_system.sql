...
-- Agregar company_id a la tabla projects existente (si no existe)
DO $$ 
BEGIN
...
-- Si la tabla projects no existe, crearla completa
CREATE TABLE IF NOT EXISTS projects (
  ...
  name VARCHAR(255) NOT NULL,
  description TEXT,
  status VARCHAR(50) DEFAULT 'backlog', -- backlog, planning, in_progress, on_hold, review, done, cancelled
...
-- Índices adicionales para projects
...
-- Si la tabla tasks no existe, crearla completa
...
-- Índices adicionales para tasks
...
-- =====================================================
-- 9. WORKSPACE - PÁGINAS (WORKSPACE_PAGES)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS workspace_pages (
  ...
  path TEXT, -- ruta jerárquica: /page1/subpage1/subpage2
  order_index INTEGER DEFAULT 0,
  is_template BOOLEAN DEFAULT FALSE,
  is_archived BOOLEAN DEFAULT FALSE,
  permissions JSONB DEFAULT '{}', -- permisos específicos de la página
  created_by UUID REFERENCES auth.users(id),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  billing_day INTEGER, -- día del mes para cobro
...
-- Función para auto-crear CompanyUser cuando se crea una Company
...
COMMENT ON TABLE companies IS 'Empresas internas del dueño (Verlyx, PulsarMoon, VOVO, etc.)';
...
COMMENT ON TABLE documents IS 'Documentos con versionado y relaciones múltiples';
COMMENT ON TABLE workspace_pages IS 'Páginas estilo Notion para cada empresa';
COMMENT ON TABLE workspace_blocks IS 'Bloques de contenido dentro de páginas workspace';
...
COMMENT ON TABLE automations IS 'Definición de workflows automatizados';
